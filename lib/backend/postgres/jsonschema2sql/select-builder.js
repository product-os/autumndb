...
module.exports = class SqlSelectBuilder {
	 ...
	 *
	 * * table - The table that `path` references.
	 * * path - Path to `ORDER BY`.
	 * * isDescending - Whether the sort direction is in
	 *   descending order. If false, the sort direction is ascending
	 *   order. Optional, defaults to false (ascending).
	 *
	 * @param {Array} orderByDef - an array containing [table, path, isDescending] (see above)
	 * @returns {SqlSelectBuilder} `this`.
	 */
	pushOrderBy (orderByDef) {
		const [ table, path, isDescending = false ] = orderByDef
