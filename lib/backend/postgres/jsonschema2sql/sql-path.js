...
module.exports = class SqlPath {
	...
	static getVersionComputedField (table) {
		const version = `CONCAT_WS('.', ${table}.version_major, ${table}.version_minor, ${table}.version_patch)`
		const vWithPreRelease = `CONCAT_WS('-', ${version}, NULLIF(${table}.version_prerelease, '') )`
		const vWithPreReleaseAndBuildSuffix = `CONCAT_WS('+', ${vWithPreRelease}, NULLIF(${table}.version_build, '') )`
