...
const createOrderByDefinitions = (table, sortBy, isDescending) => {
	...
	if (arrSortBy.length === 1 && arrSortBy[0] === 'version') {
		return [
			...
			[ table, new LiteralSql(`${table}.version_prerelease`), false ],
			[ table, new LiteralSql(`${table}.version_major`), isDescending ],
			[ table, new LiteralSql(`${table}.version_minor`), isDescending ],
			[ table, new LiteralSql(`${table}.version_patch`), isDescending ],
			[ table, new LiteralSql(`${table}.version_build`), isDescending ]
		]
	}

	return [
		[ table, SqlPath.fromArray(arrSortBy), isDescending ]
	]
}
...
const pushLinkedLateral = (
	...
	if (options.sortBy) {
		...
		for (const [ , path ] of createOrderByDefinitions('linked', options.sortBy)) {
...
module.exports = class SqlQuery {
	...
	fillOrderBy (table, select) {
		...
		for (const orderBy of createOrderByDefinitions(table, this.options.sortBy, isDescending)) {
			select.pushOrderBy(orderBy)
		}
	}

	fillInnerOrderAndGroupBy (table, innerSelect) {
		if (this.options.sortBy) {
			const orderByDefs = createOrderByDefinitions(table, this.options.sortBy, this.isDescendingSort())
			for (const orderBy of orderByDefs) {
				innerSelect.pushGroupBy(orderBy.slice(0, 2))
				innerSelect.pushOrderBy(orderBy)
			}
		} else {
