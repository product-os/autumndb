...
const createOrderByDefinitions = (table, sortBy, isDescending) => {
	...
	if (arrSortBy.length === 1 && arrSortBy[0] === 'version') {
		return [
			...
			{
				table, sql: new LiteralSql(`${table}.version_prerelease`), isDescending: false
			},
			{
				table, sql: new LiteralSql(`${table}.version_major`), isDescending
			},
			{
				table, sql: new LiteralSql(`${table}.version_minor`), isDescending
			},
			{
				table, sql: new LiteralSql(`${table}.version_patch`), isDescending
			},
			{
				table, sql: new LiteralSql(`${table}.version_build`), isDescending
			}
		]
	}
	...
	return [
		{
			table, sql: SqlPath.fromArray(arrSortBy), isDescending
		}
	]
}
...
const pushLinkedLateral = (
	...
	if (options.sortBy) {
		...
		for (const {
			sql
		} of createOrderByDefinitions('linked', options.sortBy)) {
	...
	fillInnerOrderAndGroupBy (table, innerSelect) {
		if (this.options.sortBy) {
			...
			for (const orderBy of orderByDefs) {
				...
				innerSelect.pushOrderBy(orderBy.table, orderBy.sql, orderBy.isDescending)
			}
		} else {
