...
const FragmentBuilder = require('./fragment-builder')
...
module.exports = class ExpressionFilter extends SqlFilter {
	...
	applyBinaryOperator (operator, other) {
		...
		if (otherIsExpression) {
			this.optionalLinks.push(...other.optionalLinks)
		}
		...
		if (this.expr.length === 1 && _.isBoolean(this.expr[0])) {
			...
			if (other instanceof SqlFilter) {
				if (_.isBoolean(folded)) {
					...
					optionalLinks.push(...other.optionalLinks)
				}
			}
	...
	toSqlInto (builder) {
		...
		const dummyBuilder = new FragmentBuilder(context)
