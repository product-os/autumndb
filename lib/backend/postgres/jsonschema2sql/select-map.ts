...
export class SelectMap {
	...
	toSql(table: string): string {
		...
		let selectedProperties: boolean | string[] = true;
		...
		// return
		let head: true | string[] = selectedProperties;
		...
		for (const data of conditionalBranches) {
			...
			conditionalCases.push(
				data.branch.toConditionalJsonbFragment(table, branchProperties),
			);
		}
		// Get the properties we need to include in the `ELSE` clause if it
		// exists
		let tail: true | string[] = [];
		...
		if (conditionalCases.length > 0) {
			...
			let body = util.formatMergeJsonbViewsFor(conditionalCases);
			if (sqlTail !== null) {
