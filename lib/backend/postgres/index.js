	...
	if ('links' in card) {
		...
		for (const [ linkType, linked ] of Object.entries(cardLinks)) {
			if (linked.length === 0) {
				...
				cardLinks[linkType] = linked.map((linkedCard) => {
...
module.exports = class PostgresBackend {
	...
	/*
	 * Patch a card in the database, only
	 * updating the fields that were changed.
	 */
	async patchElement (context, object, diff) {
		return cards.patch(context, this.errors, this.connection, object, diff)
	}
	 ...
	 */
	async getElementsById (context, ids) {
		/*
