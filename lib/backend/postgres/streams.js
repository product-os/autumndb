...
class Stream extends EventEmitter {
	constructor (context, streamer, id, select, schema, options) {
		...
		this.setSchema(select, schema, options, id)
	...
	setSchema (select, schema, options, newID) {
	...
	async tryEmitEvent (payload) {
		...
		try {
			...
			// TODO: figure out a way to strigger a update on other streams
			// when a message is created on a timeline, it shouldn't send setSchema events
			// to all clients listening. Instead it should update the schema each client
			// listening on the same card directly here in the backend.
			const res = this.streamer.streams.map((item) => {
				return item
			})
