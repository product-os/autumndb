...
/**
 * Build and return an array of UPDATE statements needed to replicate changes
 * made after applying JSON patch operations to a card.
 *
 * @function
 *
 * @param {Object} patchedObject - patched card object
 * @param {Object} diff - diff of pre-patch and post-patch card objects
 * @returns {Array} list of UPDATE statement objects needed to replicate patch operations in database
 *
 * @example
 * exports.toPatchUpdates(patchedObject, diff)
 * // => [
 *   {
 ...
 *       'bar',
 *       '{"name":"bar"}',
 *       '985c6e56-4bdb-11eb-9fd5-f7b91fc541a4'
 *     ]
 *   }
 * ]
 */
exports.toPatchUpdates = (patchedObject, diff) => {
	...
	// Handle added/updated fields.
	const jsonb = {}
	const columnUpdates = []
	const columnValues = []
	_.forOwn(diff, (change, key) => {
		const path = key.split('.')
		const column = path.shift()

		// Non-JSONB column updates.
		// No need to worry about additions/deletions here.
		if (_.isEmpty(path)) {
			if (change.type === 'update') {
				columnValues.push(change.value)
				columnUpdates.push(`${column}=$${columnValues.length}`)
			}
			return
		}

		// Handle JSONB field deletions.
		// These require separate UPDATE statements.
		if (change.type === 'delete') {
			queries.push({
				text: `UPDATE ${CARDS_TABLE} SET ${column} = ${column} #- ${SqlPath.toJsonSelector(path)} WHERE id=$1`,
				values: [
					patchedObject.id
				]
			})
		...
		// Build object(s) with added/updated JSONB field data only.
		// These are stringified and combined with root-level column updates in a single statement.
		if (!_.has(jsonb, column)) {
			jsonb[column] = {}
		}
		let cursor = jsonb[column]
		for (let idx = 0; idx < path.length; idx++) {
			if (idx === path.length - 1) {
				cursor[path[idx]] = _.get(patchedObject, key)
			} else {
				cursor[path[idx]] = {}
	...
	// Push added/updated JSONB field data.
	_.forOwn(jsonb, (object, column) => {
		columnValues.push(JSON.stringify(object))
		columnUpdates.push(`${column} = ${column} || $${columnValues.length}::jsonb`)
	})

	// Generate and push UPDATE statement for additions/updates.
	if (!_.isEmpty(columnUpdates)) {
		columnValues.push(patchedObject.id)
		queries.push({
			text: `UPDATE ${CARDS_TABLE} SET ${columnUpdates.join(', ')} WHERE id=$${columnValues.length}`,
			values: columnValues
		})
	}
...
exports.upsert = async (context, errors, connection, object, options) => {
	...
	const elementId = options.id || insertedObject.id || await uuid.random()
...
exports.patch = async (context, errors, connection, object, diff, options) => {
	const table = (options && options.table) ? options.table : exports.TABLE

	assert.INTERNAL(context, object.id,
		errors.JellyfishDatabaseError, 'Missing id')

	const patchedObject = Object.assign({}, object)
	patchedObject.links = {}

	patchedObject.updated_at = new Date().toISOString()
	diff.updated_at = {
		type: 'update',
		value: patchedObject.updated_at
	}

	logger.debug(context, 'Patching element', {
		table,
		slug: patchedObject.slug
	})

	// Build a list of UPDATE statements needed to patch card in database.
	const updates = exports.toPatchUpdates(patchedObject, diff)

	try {
		// Use task interface if more than one query needs to be executed.
		if (updates.length === 1) {
			await connection.any(updates[0])
		} else {
			await connection.task(async (task) => {
				const tasks = _.map(updates, (update) => {
		...
		if (/canceling statement due to statement timeout/.test(error.message)) {
			throw new errors.JellyfishDatabaseTimeoutError(`Timeout when patching ${object.slug}`)
		}

		throw new errors.JellyfishDatabaseError(error.message)
	}

	patchedObject.name = typeof patchedObject.name === 'string'
		? patchedObject.name
		: null
	utils.convertDatesToISOString(patchedObject)

	metrics.markCardUpsert(patchedObject)
