...
const CARDS_SELECT = [
	...
	'CONCAT_WS(\'+\', ' +
		'CONCAT_WS(\'-\', ' +
			'CONCAT_WS(\'.\', version_major, version_minor, version_patch)' +
			', NULLIF(version_prerelease,\'\')' +
		'), NULLIF(version_build,\'\')) AS version',
	'name',
	'tags',
	'markers',
	'created_at',
...
exports.setup = async (context, connection, database, options = {}) => {
	...
	/* To be able to do the ALTER COLUMN...NOT NULL below, we first must fix all old data.
	   The following SQL must be therefore be executed manually:
	   ...
	   -- Disable all triggers, namely for streams.
		...
		BEGIN
			LOOP
				UPDATE cards SET version_major = COALESCE(version_major, \'1\'),
					version_minor = COALESCE(version_minor, \'0\'), version_patch = COALESCE(version_patch, \'0\')
					WHERE id IN (SELECT id FROM cards WHERE version_major IS NULL OR version_minor IS NULL OR
					version_patch IS NULL
					FOR UPDATE SKIP LOCKED LIMIT 1000)
				COMMIT;
				GET DIAGNOSTICS updated = ROW_COUNT;
				IF updated = 0 THEN
					-- this time without SKIP LOCKED to be sure we're really done
					IF NOT EXISTS (SELECT FROM cards WHERE version_major IS NULL OR version_minor IS NULL OR
					  version_patch IS NULL) THEN
						EXIT;
					END IF
				END IF;
				total = total + updated;
		...
		ALTER TABLE ${table} ALTER COLUMN version_major SET NOT NULL;
		ALTER TABLE ${table} ALTER COLUMN version_major SET DEFAULT 1;
		ALTER TABLE ${table} ALTER COLUMN version_minor SET NOT NULL;
		ALTER TABLE ${table} ALTER COLUMN version_minor SET DEFAULT 0;
		ALTER TABLE ${table} ALTER COLUMN version_patch SET NOT NULL;
		ALTER TABLE ${table} ALTER COLUMN version_patch SET DEFAULT 0;
	...
	*/
	await connection.any(`
		...
		-- old constraint was not named - this SHOULD be the auto-generated one
		ALTER TABLE ${table} DROP CONSTRAINT IF EXISTS cards_slug_version_major_version_minor_version_patch_key;
...
exports.upsert = async (context, errors, connection, object, options) => {
	...
	const version = parseVersion(insertedObject.version)
