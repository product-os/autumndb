...
exports.setup = async (context, connection, database, options) => {
	...
	if (!indexes.includes(slugVersionIndex)) {
		await utils.createIndexConcurrently(context, connection, LINK_TABLE, slugVersionIndex,
	...
	for (const [ name, column ] of [
		...
		await utils.createIndexConcurrently(context, connection, LINK_TABLE, name, `USING BTREE (${column})`)
	}
